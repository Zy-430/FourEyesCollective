...
// Fetch items in the order that are not yet rated
$stm_items = $_db->prepare("
    SELECT oi.order_item_id, p.product_name, p.product_image
    FROM order_item oi
        ...
        <div id="rateItems" class="rate-items">
            <?php foreach ($items as $item): ?>
                <div class="rate-card" data-order-item-id="<?= encode($item['order_item_id']) ?>">
                    ...
                    <img src="/images/product/<?= encode($item['product_image']) ?>" alt="<?= encode($item['product_name']) ?>" class="rate-thumb">
    ...
    $(document).ready(function() {
        ...
        $('.rate-card').each(function() {
            ...
            $card.find('.submitRate').on('click', function() {
                ...
                $.ajax({
                    url: '/page/order_rate_submit.php',
                    type: 'POST',
                    data: formData,
                    processData: false,
                    contentType: false,
